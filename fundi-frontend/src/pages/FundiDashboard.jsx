...
const mockJobs = [
  { id: 1, title: 'Fix Sink', location: 'Nairobi', skill: 'Plumbing', description: 'Leaking kitchen sink', status: 'Open' },
  { id: 2, title: 'Paint Living Room', location: 'Kisumu', skill: 'Painting', description: 'Need a painter', status: 'Open' }
];
          ...
          <ul className="space-y-4">
            {jobs.map(job => (
              <li key={job.id} className="bg-white rounded shadow p-4 flex justify-between items-center">
                <div>
                  <span className="font-bold">{job.title}</span> - {job.skill} <span className="ml-2 text-gray-500">({job.location})</span>
                  <div className="text-sm text-gray-500">{job.description}</div>
                </div>
                <button className="bg-green-600 text-white px-2 py-1 rounded">Apply</button>
              </li>
            ))}
          </ul>
        </div>
